...
class SpotPlotLP(Row):
    ...
    '''
    A SpotPlot and a line plot of the data against z at the x and y coordinates linked to a
    custom tap tool.
    '''
    ...
    def __init__(self,x,y,z,D,palette = 'Viridis256',cfile = 'jet.txt',
        ...
        '''
        All init arguments same as for SpotPlot except for additional arguments:
        ...
        updateLP = CustomJS(args={'source':self.lpds},code=jscode)
        ttool = TapTool(callback = updateLP)

        self.spplot = SpotPlot(x,y,z,D,palette=palette,cfile=cfile,xlab=xlab,ylab=ylab,zlab=zlab,Dlab=Dlab,
                               height=spheight,width=spwidth,rMin=rMin,rMax=rMax,
                               xran=xran,yran=yran,ttool=ttool)
        ...
        if revz: self.lplot.y_range.start,self.lplot.y_range.end = \
